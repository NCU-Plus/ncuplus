describe("courses index page", () => {
  ...
  it("type 課名搜尋", () => {
    ...
    cy.get(".row-title").eq(0).should("have.text", "軟體工程實務");
  });
  ...
  it("type 課號搜尋", () => {
    ...
    cy.get(".row-title").eq(0).should("have.text", "軟體工程實務");
    cy.get(".row-subtitle").eq(0).should("have.text", "梁德容, 王尉任, 鄭永斌");
    cy.get(".row-description").eq(0).should("have.text", "資訊工程學系");
  });
  ...
  it("type 老師搜尋", () => {
    ...
    cy.get(".row-subtitle").eq(0).should("contain", "梁德容");
  });
    ...
    cy.get("select")
      ...
      .each(($option, index) => {
        if (index == 0) {
          ...
          cy.wait(1000);
          cy.get(".row-description").eq(0).should("contain", "體育室");
        } else {
          cy.get("select").eq(1).select(Cypress.$($option).text());
          cy.wait(1000);
          cy.get(".row-description")
            .eq(-1)
  ...
  it("click Last", () => {
    ...
    cy.wait(1000);
    cy.contains("Last").click();
    cy.get("#page-input").should("have.value", "4");
  });
